...
class ParticleFilter:
    ...
    def predict(self, u):
        ...
        for p in range(0, self.N_SAMPLES): # this is not very Pythonic, feel free to improve...
            # predict mean:
            c_theta = np.cos(self.particles[p,2])
            s_theta = np.sin(self.particles[p,2])
            x_dot = c_theta * v
            y_dot = s_theta * v
            theta_dot = omega
            self.particles[p,:] += dt * np.array([x_dot, y_dot, theta_dot])
            # add noise:
            eta_l = np.random.normal(0,ANGULAR_NOISE/dt)
            eta_r = np.random.normal(0,ANGULAR_NOISE/dt)
            vel_noise = (eta_l+eta_r)*0.5*CAR_WHEEL_RADIUS
            self.particles[p,0] += c_theta * vel_noise
            self.particles[p,1] += s_theta * vel_noise
            self.particles[p,2] += (eta_r-eta_l)*CAR_WHEEL_RADIUS/CAR_WHEEL_BASE
            # wraparound:
    ...
    def update(self, z):
        # Bayesian update:
        has_observed_landmark = np.zeros(self.landmarks.shape[0], dtype=bool)
        for p in range(0, self.N_SAMPLES): # this is not very Pythonic, feel free to improve...
            x = self.particles[p]
            ...
            for (i, zi_tilde) in z:
                ...
                # Compute the expected measurement if the landmark is in front of the camera otherwise set to a large value (out of the image) for low likelihood
                if lm_B[0]>1.0e-10:
                    zi = (-CAM_F*lm_B[1]/lm_B[0]+CAM_C).item()
                    # Check if the expected measurement is within the image bounds, otherwise set to a large value (out of the image) for low likelihood
                    if zi < 0.5 or zi > CAM_W-0.5: 
                        zi = 10000
                    else:
                        has_observed_landmark[i] = True
                else:
                    zi = 10000
                ...
                if np.isnan(P):
                    P = 1.0e-10
                self.weights[p] *= P
                if np.isnan(self.weights[p]):
                    self.weights[p] = 1.0e-10

        if np.sum(has_observed_landmark) == 0 and len(z) > 0: # if we have observed some measurements but none of the particles are able to see any of the observed landmarks, we count this as a failed update
            self.consecutive_failed_updates += 1
        else:
            self.consecutive_failed_updates = 0

        # if we have too many consecutive failed updates (meaning none of the particles are able to see any of the observed landmarks)
        # we reset the filter
        if self.consecutive_failed_updates > 3:
            self.need_initialisation = True
            return
        ...
        if W == 0.0:
            ...
            self.weights *= 1.0/W
            self.weights[np.isnan(self.weights)] = 1.0e-10

        self.update_counter += 1
        if self.update_counter % RESAMPLE_EVERY_N_UPDATES == 0:
            # re-sample:
            newparticles = self.particles.copy()

            # Resampling the particles according to their weights (with some noise added to the particles to keep diversity) 
            index = np.random.choice(len(self.particles), len(self.particles), p=self.weights.flatten())
            for i in range(self.N_SAMPLES):
                newparticles[i,:] = self.particles[index[i],:] + np.array([np.random.normal(0,RESAMPLING_NOISE_XY), np.random.normal(0,RESAMPLING_NOISE_XY), np.random.normal(0,RESAMPLING_NOISE_THETA)])
    ...
    def getStateAndCov(self):
        x = np.sum(self.particles[:,0]*self.weights.flatten())
        y = np.sum(self.particles[:,1]*self.weights.flatten())
        sin_sum = np.sum(np.sin(self.particles[:,2])*self.weights.flatten())
        cos_sum = np.sum(np.cos(self.particles[:,2])*self.weights.flatten())
...
class Ekf:
    ...
    def predict(self, u):
        ...
        omega = (u[1]-u[0])*CAR_WHEEL_RADIUS/CAR_WHEEL_BASE
        # predict mean:
        s_theta = np.sin(self.x[2])
        c_theta = np.cos(self.x[2])
        x_dot = c_theta * v
        y_dot = s_theta * v
        theta_dot = omega
        ...
        # predict covariance:
        F = np.array([[1.0, 0.0, -dt*v*s_theta],
                      [0.0, 1.0, dt*v*c_theta],
                      [0.0, 0.0, 1.0]])
        L = np.array([[c_theta*0.5*CAR_WHEEL_RADIUS, c_theta*0.5*CAR_WHEEL_RADIUS],
                      [s_theta*0.5*CAR_WHEEL_RADIUS, s_theta*0.5*CAR_WHEEL_RADIUS],
                      [CAR_WHEEL_RADIUS/CAR_WHEEL_BASE, -CAR_WHEEL_RADIUS/CAR_WHEEL_BASE]])
        Q = np.array([[(ANGULAR_NOISE/dt)**2, 0.0],[0.0, (ANGULAR_NOISE/dt)**2]])
    ...
    def update(self, z):
        ...
        for (i, zi_tilde) in z:
            ...
            Hi[0,2] = np.matmul(U,np.array([[-np.sin(x[2])*dx+np.cos(x[2])*dy],[-np.cos(x[2])*dx-np.sin(x[2])*dy]])).item()
            zi = (-CAM_F*lm_B[1,0]/lm_B[0,0]+CAM_C).item()

...
class PoseEstimator:
    ...
    def estimate(self, z, x0):
        """
        Refine pose x = [x_R, y_R, θ] from a LiDAR scan z using Gauss-Newton.
        Implement according to Task 1 (i)–(iii).
        """
        x = x0.copy().astype(float)
        for _ in range(MAX_GN_ITER):
            dx = np.zeros_like(x) # TODO: replace with the correct Gauss-Newton update derived in (iv)
...
class ETHDriveSimulator:
    ...
    def __init__(self, mask, debug=False):
        ...
        self.key_state     = np.zeros(2)
        self.lidar_hits    = None
    ...
    def update(self, dt):
        ...
        if self.frame % REFINE_EVERY == 0:
            ...
            # Freeze LiDAR hit positions in world frame until next scan
            th = self.est_pose[2]

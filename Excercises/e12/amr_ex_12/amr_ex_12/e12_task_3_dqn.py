...
from amr_ex_12.utils import QNetwork, ReplayBuffer
...
class DQNTrainer:
    ...
    def predict(
        ...
        with torch.no_grad():
            # TODO: Predict the optimal action according to the `q_net`.
    ...
    def learn(self, total_timesteps: int) -> "DQNTrainer":
        ...
        for global_step in range(total_timesteps):
            ...
            if len(self.memory) > self.learning_starts:
                b_s, b_a, b_r, b_ns, b_d = self.memory.sample(self.batch_size, self.device)

                # TODO: Get Q values associated with b_s and b_a.
                # Should have size (batch_size,)
                current_q = ...

                with torch.no_grad():
                    # TODO: Compute the Value function at b_ns (i.e. max over action of Q at b_ns).
                    # How does performance change if you use the `q_net` vs the `target_net` to perform
                    # the maximization?
                    max_next_q = ...
                    # TODO: use the Value function at b_ns to compute the expected q value
                    expected_q = ...

                # TODO: Compute the loss between your current Q value estimate and the expected estimate
                loss = ...
